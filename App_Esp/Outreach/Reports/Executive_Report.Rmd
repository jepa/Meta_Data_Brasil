...
comunidad o región. Desde noviembre de 2016 se han creado `r N_entries` registros, colectando información sobre más de `r N_dp` mil datos generados entre `r Y_Inicio` y 2017, siendo el océano Pacífico el litoral con más información hasta ahora. Dichos datos provienen de más de `r N_rep` fuentes de libre acceso y publicaciones científicas siendo pesquerías y ecología las disciplinas con más registros. Si omitimos los registros que contienen múltiples especies, encontramos que `r Especies$Subject_name[2]`  y `r Especies$Subject_name[3]` son los organismos con más registros y las localidades con más registros son `r Localidades$Location[2]` y `r Localidades$Location[3]`. A pesar de tener una respuesta positiva de más de 60 individuos de la comunidad científica y distintas instituciones gubernamentales y no gubernamentales, aún falta información por recopilar Cuantas más personas e instituciones aporten información a la base de metadatos, mejor se podrá reflejar el estado de la investigación marina en México.
...
```{r Fig3, echo=F,eval=F,message=F,warning=F, fig.height=3, fig.width=6, fig.cap="Historial de datos capturados en los metadatos. Nota, los datos futuros representan modelos de proyección"}
...
```{r Fig1, echo=F,eval=F, fig.height=3, fig.width=6, fig.cap= "Registros por Litoral Mexicano. Nacional se refiere a datos que comprenden 2 o más litorales. Terrestre se refiere a datos en estados sin litoral"}
